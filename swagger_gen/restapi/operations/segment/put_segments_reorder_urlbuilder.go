...
func (o *PutSegmentsReorderURL) Build() (*url.URL, error) {
	...
	var _path = "/flags/{flagID}/segments/reorder"

	flagID := swag.FormatInt64(o.FlagID)
	if flagID != "" {
		_path = strings.Replace(_path, "{flagID}", flagID, -1)
	} else {
		return nil, errors.New("FlagID is required on PutSegmentsReorderURL")
	}
...
func (o *PutSegmentsReorderURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PutSegmentsReorderURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PutSegmentsReorderURL")
	}
